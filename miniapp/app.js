...
(() => {
  ...
  const GLOBAL_SCHEMA = {
    "🔍 Фильтр пар": [
      ...
      { k: "filter.min_pair_wr_actual", t: "int", min: 0, max: 80, step: 1, label: "Мин. WR пары из аналитики (%)",
        desc: "Бот считает counterfactual WR каждой пары из таблицы signals (за последние 30 дней, 2-бар экспирация). Если WR < этого значения — пара НЕ торгуется (но в аналитику пишется через broad pool). Требуется минимум 30 сигналов чтобы порог применился — иначе пара получает шанс. 0 = выключено." },
      { k: "filter.auto_expiry_enabled", t: "bool", label: "Авто-экспирация per-pair × hour",
        desc: "Каждые 4 часа бот пересчитывает оптимальную экспирацию (1-5 баров) для каждой пары в каждом часе из counterfactual exp_wins данных. При открытии сделки берётся оптимум для (sym, current_hour), fallback на trading.expiry_seconds если данных <8. Может давать +5-12 п.п. WR на парах где не-2-бара лучше." },
    ...
    "💰 Торговля": [
      ...
      { k: "trading.expiry_seconds", t: "int", min: 30, max: 600, label: "Экспирация (сек)",
        desc: "длительность открытой сделки в секундах (60 = 1 мин, 120 = 2 мин). При вкл auto_expiry — fallback значение." },
      { k: "trading.parallel_pairs", t: "bool", label: "⚡ Параллельная торговля несколькими парами",
        desc: "Stage 3: бот ведёт НЕЗАВИСИМЫЕ МГ-циклы на нескольких парах одновременно. Каждый цикл закрывается своим WIN. Сигнал на одной паре не блокирует открытие на другой. Если выключено — legacy single-pair режим (одна пара за раз)." },
      { k: "trading.max_parallel_pairs", t: "int", min: 1, max: 8, label: "Макс. параллельных пар", parent: "trading.parallel_pairs",
        desc: "Максимум одновременно активных МГ-циклов. По дефолту 3. Капитал-аспект: при N циклах max-риск одной свечи = N × stake[mg_step] × (потенциал bustа). Например 3 цикла × MG5 = $108 риск одновременно при base=$1." },
    ],
    "🎰 Мартингейл": [
      ...
      { k: "martingale.pair_limits", t: "intlist", label: "Сделок на каждой паре через запятую (длина = число пар)", parent: "martingale.enabled",
        desc: "Пример «3,3,2» = 3 пары в цикле (1-я до 3 сделок, 2-я до 3, 3-я до 2). При switch с не-последней пары РАНЬШЕ исчерпания лимита (например payout упал) — неиспользованные перекрытия идут в РЕЗЕРВ и передаются ПОСЛЕДНЕЙ паре цикла. На последней паре с опцией «торговать до stop-sum» — все лимиты и payout-проверки игнорируются, добивает цикл до WIN или stop_sum (резерв формально присутствует но не нужен — последняя и так безлимитна)" },
      { k: "martingale.carry_unused", t: "bool", label: "Переносить неиспользованные перекрытия в резерв", parent: "martingale.enabled",
        desc: "Если бот ушёл со 2-й пары после 2 сделок при лимите 3, в резерв упадёт 1. Все накопленные резервные сделки реализуются ИСКЛЮЧИТЕЛЬНО на последней паре цикла. Если у последней включён режим «торговать до stop_sum» — резерв формально не нужен (последняя безлимит). Без режима «до stop_sum» — последняя получит свой_лимит + резерв" },
      { k: "martingale.last_pair_until_stop_sum", t: "bool", label: "На последней паре цикла торговать до stop-sum", parent: "martingale.enabled",
        desc: "На последней паре цикла игнорировать лимит pair_limits и payout-drop, торговать до WIN или до stop_sum. Все накопленные резервные сделки реализуются здесь — фактически последняя пара становится «безлимитной» (ограничена только cycle_total_limit и stop_sum). Выкл = последняя пара ограничена своим лимитом + резервом" },
