...
### Шаг 2.5. In-cycle SEARCH — ослабленный фильтр

Когда бот **уже в цикле** (mg_step > 0) и происходит switch на следующую пару (по исчерпанию лимита или payout-drop), фильтр кандидатов работает мягче:

| Триггер | FREE-режим (вход в цикл) | CYCLE SEARCH (добивание) |
|---|---|---|
| `score.ban` (max_loss_streak) | ❌ блокирует | ❌ блокирует |
| `score.pause` (recent_wr1 fail) | ❌ блокирует | ✅ **ИГНОРИРУЕТСЯ** |
| `score.temp_pause` (обе провалены) | ❌ блокирует | ✅ **ИГНОРИРУЕТСЯ** |
| `payout < min_payout` | ❌ блокирует | ❌ блокирует |
| `switched_pairs` (уже использована) | n/a | ❌ блокирует (anti-bounce) |

**Логика**: в CYCLE мы уже инвестировали деньги в предыдущую пару, нужно отбить — текущая форма пары (recent WR1) вторична по сравнению с экспирацией цикла. recent_wr1 фильтр применяется только при ВХОДЕ в цикл, не при добивании.

### Sticky `current_pair`

Пара, на которой **сейчас активен MG-цикл**, принудительно остаётся в `_tracked` при каждом rescan, даже если её оценка ухудшилась во время цикла. Это нужно чтобы цикл мог довестись до WIN/stop_sum на этой паре (без неё `_in_cycle_step` потерял бы доступ к свечам).
