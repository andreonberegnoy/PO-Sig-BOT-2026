...
- **Загруженные user-стратегии** (Python-файлы) — попадают в `/app/data/user_strategies/*.py`, не в образ контейнера. Папка переключается автоматически: если есть `/app/data` → volume, иначе → `strategy/user/` (для локальной разработки).
