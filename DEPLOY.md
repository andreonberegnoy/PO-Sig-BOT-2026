...
Опциональное окно в течение которого бот заходит в новые сделки. Настраивается в Mini App → вкладка «Настройки» → раздел «⏰ Расписание работы»:

- **Toggle «Работать по расписанию (снять = 24/7 круглосуточно)»** — главный переключатель. По умолчанию **выключен** (бот торгует круглосуточно).
- **Час начала / Час конца** — границы окна в твоей локальной таймзоне (`telegram.daily_report_timezone`, по умолчанию `Europe/Kyiv`).
- Окно через полночь поддерживается: `start=22`, `end=6` означает с 22:00 до 06:00.

**Поведение при активном расписании:**

1. **В рабочем окне** → нормальная торговля.
2. **Конец окна наступил во время мартингейл-цикла** → бот **обязан довести цикл до WIN** (никогда не бросает мартингейл вне зависимости от расписания).
3. **Цикл закрылся вне окна (WIN)** → бот авто-паузится с флагом `auto_paused_schedule=True` и шлёт в TG: «🌙 Закрыл цикл — пора отдыхать. Хороший день был…»
4. **Наступил `start_hour` следующего дня** → если `auto_paused_schedule=True`, бот авто-возобновляется и шлёт «☀️ Доброе утро. Рабочее окно открылось — возвращаюсь к торговле.» Manual `/pause` оставляет паузу до явного `/resume` (флаг `auto_paused_schedule` остаётся False).

**Что НЕ останавливается при паузе/вне окна:**
- Логгеры аналитики (см. ниже)
- Buffer keeper (догрузка свечей)
- WS-соединение и live-тики
- Auto-relogin
- Telegram-команды
...
**Toggle «Только рабочие часы (по расписанию)»** — фильтрует данные на стороне отображения по часовому окну `schedule.start_hour..end_hour` в твоей таймзоне. Фильтрация делается через SQL `strftime('%H', ts + tz_offset, 'unixepoch')`, поэтому быстрая даже на большой истории.

**Важно**: сами логгеры пишут **24/7 независимо от расписания**. Фильтр применяется только к отображению — так в БД всегда есть данные в обоих режимах, можно сравнивать поведение пары днём vs ночью без перенакопления.
