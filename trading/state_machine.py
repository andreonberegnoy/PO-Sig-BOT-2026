...
class StateMachine:
    ...
    async def backfill_history(self, payout_default: int = 92,
                                limit_pairs: Optional[int] = None) -> dict:
        """Прогоняет активную стратегию по всем cached candles каждой
        известной пары и вписывает signals + market snapshots + exp_wins.
        Идемпотентно (UNIQUE constraint в БД).

        Параметры:
          payout_default: PO не отдаёт исторические выплаты — стампим
            фиксированное значение на все исторические сигналы (default 92).
          limit_pairs: ограничить число пар (для тестов).

        Возвращает сводку {pairs_processed, total_signals, total_inserted,
        total_settled, per_pair: {sym: {signals, inserted, settled}}}.
        """
        ...
        try:
            ...
            return {"error": "active strategy unavailable"}

        # Все известные пары — _pair_scores заполняется на каждом _rescan_pairs.
        # ИСКЛЮЧАЕМ 12-часовые баны (макс. потерь подряд) — их паттерн
        # явно деструктивен и в обучающую выборку фильтра не должен попадать.
        # paused-пары (короткая 1ч пауза за recent WR1) включаем — это
        # временная просадка, не фундаментальная.
        all_syms = list((self._pair_scores or {}).keys())
        if not all_syms:
            all_syms = list(self._tracked or set())
        syms: list[str] = []
        skipped_banned = 0
        for s in all_syms:
            if self.journal.is_banned(s):
                skipped_banned += 1
                continue
            syms.append(s)
        if limit_pairs:
            syms = syms[:int(limit_pairs)]

        tf = int(self.cfg.get("filter", {}).get("tf", 60))
        total_sigs = total_ins = total_settled = 0
        ...
        for sym in syms:
            try:
                ...
                if len(candles) < 200:
                    try:
                        fresh = await _fetch_candles(self.feed, sym, tf, 1500)
                        if fresh and len(fresh) > len(candles):
                            candles = fresh
                    except Exception:
                        logger.exception("backfill: fetch_candles failed for %s", sym)
                if len(candles) < 200:
                    per_pair[sym] = {"error": "not enough candles",
                                     "candles": len(candles)}
                    continue

                # Используем только закрытые бары (последний может быть still-forming)
                closed_bars = candles[:-1] if len(candles) >= 2 else candles
                try:
                    sigs, _ = strat.generate_signals(closed_bars, params)
                except Exception:
                    logger.exception("backfill: generate_signals failed for %s", sym)
                    per_pair[sym] = {"error": "strategy raised"}
                    continue

                inserted = settled = 0
                for sig in sigs:
                    try:
                        snap = self._market_snapshot(sym, sig, params,
                                                     closed_bars, strat_name)
                    except Exception:
                        continue
                    if not snap:
                        continue
                    # PO не отдаёт исторические выплаты — стампим default,
                    # чтобы signal не отбраковывался фильтром по `payout_min`.
                    snap["payout_at_signal"] = int(payout_default)
                    sid = self.journal.insert_signal(snap)
                    if not sid:
                        continue   # дубликат, уже был
                    inserted += 1
                    # exp_wins по next 5 close (если в истории есть 5 баров)
                    entry_close = float(snap["entry_close"])
                    side = snap["side"]   # call | put
                    ew: list = []
                    for k in range(1, 6):
                        idx = sig.i + k
                        if idx >= len(closed_bars):
                            break
                        cl = float(closed_bars[idx]["close"])
                        if cl == entry_close:
                            ew.append(None)
                        elif side == "call":
                            ew.append(1 if cl > entry_close else 0)
                        else:
                            ew.append(1 if cl < entry_close else 0)
                    if len(ew) == 5:
                        self.journal.settle_signal(sid, ew)
                        settled += 1

                per_pair[sym] = {"signals": len(sigs),
                                 "inserted": inserted,
                                 "settled": settled,
                                 "candles": len(closed_bars)}
                total_sigs += len(sigs)
                total_ins += inserted
                total_settled += settled
                logger.info("backfill %s: %d signals, %d inserted, %d settled",
                            sym, len(sigs), inserted, settled)
            except Exception:
                logger.exception("backfill: failed for %s", sym)
                per_pair[sym] = {"error": "exception"}

        result = {
            "strategy": strat_name,
            "pairs_processed": len(syms),
            "pairs_skipped_banned": skipped_banned,
            "total_signals": total_sigs,
            "total_inserted": total_ins,
            "total_settled": total_settled,
            "payout_default": int(payout_default),
            "per_pair": per_pair,
        }
        logger.info("backfill done: pairs=%d signals=%d inserted=%d settled=%d",
                    len(syms), total_sigs, total_ins, total_settled)
        return result
