...
def create_app(*, cfg: dict, config_path: str, registry, sm, feed, journal, bot_token: str,
    ...
    async def expiry_stats(request: Request):
        ...
        if scope not in ("tracked", "all"):
            scope = "tracked"
        # source=backtest (default, CONSENSUS на свечах)
        #      | trades (real trades + exp_wins, узкая выборка)
        #      | virtual (все CONSENSUS-сигналы что бот ВИДЕЛ на tracked парах,
        #                включая пропущенные из-за LOCKED — полная картина стратегии)
        source = (request.query_params.get("source") or "backtest").lower()
        if source not in ("backtest", "trades", "virtual"):
            source = "backtest"
        try:
        ...
        # ── source=virtual branch: aggregate from virtual_signals table ──
        # Всё что CONSENSUS обнаружил на tracked парах (даже когда бот не входил)
        if source == "virtual":
            ...
            if not symbols_to_query and journal:
                cur = journal.conn.execute(
                    "SELECT DISTINCT symbol FROM virtual_signals WHERE exp_wins IS NOT NULL"
                )
            ...
            for sym in symbols_to_query:
                ...
                if trades_window > 0:
                    rows = journal.conn.execute(
                        "SELECT exp_wins FROM virtual_signals "
                        "WHERE symbol=? AND exp_wins IS NOT NULL "
                ...
                for exp_idx, exp in enumerate(EXPIRIES):
                    ...
                    per_expiry[exp] = {
                        "signals": completed, "wins": wins, "losses": losses,
                        "wr": round(wr, 1), "wr1": round(wr, 1), "max_streak": 0,
                    }
                ...
                results.append({
                    "symbol": sym, "payout": payout, "expiries": per_expiry,
                    "best_expiry": best_exp, "best_wr": best_wr,
                    "candles_used": 0, "completed_1000": len(rows),
                })
            results.sort(key=lambda r: (-r["best_wr"], -r["payout"]))
            agg = {}
            for exp in EXPIRIES:
                wr_w_num = 0.0; wr_w_den = 0
                tw = tl = ts = 0; pwd = 0
                for r in results:
            ...
            note = (
                f"Анализ {len(results)} пар ({scope_label}) по ВСЕМ сигналам стратегии. "
                f"Окно: {window_label} сигналов на пару. Пропущено (нет данных): {skipped}. "
                f"Включает сигналы что бот не взял (LOCKED, MG-цикл и т.д.) — "
                f"полная картина стратегии независимо от состояния бота."
            )
            return {
